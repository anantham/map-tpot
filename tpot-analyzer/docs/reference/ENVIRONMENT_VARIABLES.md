...
| `TPOT_CURATOR_TOKEN` | (none) | `curator_auth.py` | **Required in production.** Token for curator-private routes, including direct account-tag reads/writes, tag-derived cluster summaries and GRF affinities, and Community Gold, community, branch, seed, and graph-setting writes. Routes fail closed: unset returns 503; a missing/mismatched `X-TPOT-Curator-Token` returns 401. Account-tag mutations additionally require `X-TPOT-Curation-Source` (`human_curator_api`, `agent_assisted_curator`, or `verification_script`). |
