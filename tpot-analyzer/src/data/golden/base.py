    ...
    def ensure_fixed_splits(self, axis: str, *, assigned_by: str, force_reassign: bool = False) -> Dict[str, int]:
        ...
        with self._open() as conn:
            ...
            for row in rows:
                conn.execute(
                    """
                    INSERT OR REPLACE INTO curation_split (tweet_id, axis, split, assigned_by, assigned_at)
                    VALUES (?, ?, ?, ?, ?)
                    """,
    ...
    def list_candidates(
        ...
        *,
        ...
        if status == "labeled":
            ...
            conditions.append("ls.id IS NULL")
        ...
        query = f"""
            ...
            ORDER BY (t.created_at IS NULL) ASC, t.created_at DESC, t.tweet_id DESC
            LIMIT ?
        """
                ...
                result.append(
                    {
                        ...
                        "split": str(row["split"]),
                        "labelStatus": "labeled" if row["active_label_set_id"] is not None else "unlabeled",
