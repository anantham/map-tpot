...
by src/api/cluster/state.py from parquet data we control — never from untrusted sources.
"""
...
def load_adjacency_cache(path: Path | None = None) -> sp.csr_matrix:
    """Load the cached adjacency matrix (built by src/api/cluster/state.py on startup).
