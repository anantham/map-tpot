"""Calculate per-community seed-holdout sensitivity for @adityaarpitha.

Runs propagation multiple times with different 20% seed omissions to show
how sensitive each community score is. This is not bootstrap resampling or a
calibrated confidence-interval procedure.
"""
...
def main():
    print(f"--- Seed-Holdout Sensitivity for @adityaarpitha ({ADITYA_ID}) ---")
    ...
    # 3. Run repeated seed-holdout analysis (10 iterations)
    n_iterations = 10
    all_scores = []
    
    print(f"Running {n_iterations} seed-holdout iterations (20% omitted per run)...")
    ...
    range_low = np.percentile(all_scores, 2.5, axis=0)
    range_high = np.percentile(all_scores, 97.5, axis=0)
    ...
    for i in range(K):
        results.append({
            ...
            "Mean Score": means[i],
            "Std Dev": stds[i],
            "2.5% Rerun Quantile": range_low[i],
            "97.5% Rerun Quantile": range_high[i],
    ...
    print("\n=== PER-COMMUNITY SEED-HOLDOUT SENSITIVITY ===")
    print(f"{'Community':<30} {'Score':<8} {'Stability':<10} {'2.5–97.5% rerun range':<25}")
    print("-" * 80)
    for _, row in df.iterrows():
        range_str = (
            f"[{row['2.5% Rerun Quantile']:.2f}, "
            f"{row['97.5% Rerun Quantile']:.2f}]"
        )
        print(f"{row['Community']:<30} {row['Mean Score']:<8.2f} {row['Stability']:<10.2f} {range_str:<25}")
