...
function Discovery({ initialAccount = DEFAULT_ACCOUNT, onAccountStatusChange }) {
  ...
  // Model settings management (draft, save, analysis polling)
  const {
    modelSettings, modelSettingsDraft, modelSettingsDirty, savingModelSettings,
    analysisStatus, analysisPolling,
    handleAlphaChange, handleDiscoveryWeightChange, handleModelNumberChange,
    handleToggleAutoShadow, handleSaveModelSettings, handleRunAnalysis,
    applyServerSettings,
  } = useModelSettings({ onSettingsSaved: () => resetQueryState() })

  // Recommendation fetching, progressive loading, and caching
  const {
